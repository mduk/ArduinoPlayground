...
namespace mdns {
  ...
  bool setStationHostname(const char* p_pcHostname) {
    ...
    if (p_pcHostname) {
      ...
      Serial.printf("mdns::setStationHostname - Station hostname is set to '%s'\r\n", p_pcHostname);
    }
    return true;
  }
  ...
  */
  void hostProbeResult(String p_pcDomainName, bool p_bProbeResult) {
    ...
    Serial.printf("mdns::hostProbeResult - Host domain '%s.local' is %s\r\n", p_pcDomainName.c_str(), (p_bProbeResult ? "free" : "already USED!"));
    if (true == p_bProbeResult) {
      ...
      if (!bHostDomainConfirmed) {
        ...
        if (!hMDNSService) {
          // Add a 'clock.tcp' service to port 'HTTP_LISTEN_PORT', using the host domain as instance domain
          hMDNSService = MDNS.addService(0, "espclk", "tcp", HTTP_LISTEN_PORT);
          if (hMDNSService) {
            // Add a simple static MDNS service TXT item
            MDNS.addServiceTxt(hMDNSService, "port#", HTTP_LISTEN_PORT);
      ...
      if (MDNSResponder::indexDomain(pcHostDomain, "-", 0)) {
        ...
        Serial.println("mdns::hostProbeResult - FAILED to update hostname!");
      }
    }
  }
